...
* Add languages by adding a new object to TRANSLATIONS
* ═══════════════════════════════════════════════════════════════════════════ */
const TRANSLATIONS = {
/* ─────────────────────────────────────────────────────────────────────────────
ITALIAN
...
// ── Functional templates (section N/A) ───────────────────────────────────
...
// Global shorthand — works everywhere, updates when language changes
function t(...args) { return i18n ? i18n.t(...args) : args[0]; }
// React hook for i18n — triggers re-render on language change
function useLang() {
...
// Skip entire section if marked N/A
if (sd.sectionNA)
return "";
const itemsHtml = (sec.items || [])
.filter(item => sd.items[item.id] !== "na") // skip N/A items in PDF
.map(item => {
...
.filter(m => sd.measures[m.id] !== "na") // skip N/A measures in PDF
.map(m => {
...
${ /* section notes are internal only — not shown in client PDF */""}
${itemsHtml}
...
// Mark entire section as N/A (all items = "na", all measures = "na")
...
if (sd.sectionNA)
continue; // whole section N/A
// Any measure out of range → fail (skip N/A measures)
...
* Job → Preventivo: pulsante nel job crea una quotazione pre-compilata
...
const { lang, setLang } = useLang(); // t() is global via i18n
